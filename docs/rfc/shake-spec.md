...
- [x] <!-- REVIEW-11 (resolved): `get` loses information (F10): it answers `""` both for a name nothing bound and for `--name=`. Decision: `get(m, name) -> Maybe<String>`, `Some` of the last value bound, `None` when nothing bound it, as clap's `get_one` answers an `Option`. `on` is true exactly when `get` is `Some{"true"}`. SHAKE-GET-1 is reworded. -->
- [x] <!-- REVIEW-12 (resolved): A missing option value is reported as a missing required argument (F9), so `greet --name` says "the following required argument was not provided". Decision: a new error `NoValue{at, name}` for an option whose value never came, whether the words end or a flag-shaped word follows, like clap's "a value is required for '--name <NAME>' but none was supplied"; `Missing` is only for a required argument. SHAKE-TOK-6 is reworded. -->
...
A tagged law's statement changes only when its SPEC.md row changes, and that is a behavior change the PR says it is. `main.bend` is the interface (REVIEW-2): removing or retyping one of its defs is a breaking change, made when the design is better for it and said so in the PR. Untagged laws, proofs and everything under `src/` may change freely (REVIEW-15).
...
| A flag or single-valued option given twice is refused with `Repeated`; `many` declares an option that repeats (REVIEW-4, reversed) | SHAKE-PARSE-10 | behavior, new API |
| `get` answers `Maybe<String>` (F10, REVIEW-11) | SHAKE-GET-1 | API change |
| A missing option value is `NoValue`, not `Missing` (F9, REVIEW-12) | SHAKE-TOK-6 | behavior |
...
**Refuse ill-formed specs inside `parse`.** It would make every row unconditional. But an ill-formed spec is the program author's bug, fixed once, not an input a user can give, and checking it inside `parse` charges every run for a fact that never changes between runs. `check` answers it once, and the rows state what holds for a spec it passes.
...
**Laws that name the walker.** The walker rows' laws state their premises over the walker's state (shake-walker-proofs.md, REVIEW-W1), so restructuring the walker means re-proving them, though not restating the rows. We accept that cost: the rows, over `main.bend`, are what stays fixed.
...
**Encoding accidents as requirements.** TOK-7's scoping and F11's rule that a subcommand name always selects are today's behavior. We keep them because they are the right reading, and clap's; a row is a promise, and the maintainer should strike any row that promises more than they want to keep.
