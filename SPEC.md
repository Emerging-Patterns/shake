...
| SHAKE-TOK-7 | A long or short spelling that no argument of the current command has is refused as `UnknownFlag{word}`. Arguments of a parent command are not matched after a subcommand is selected. | Proved | pending | src/LAWS.bend unknown_long; src/LAWS.bend unknown_short; src/LAWS.bend enter_step |
...
| SHAKE-TOK-7 | `unknown_long`, `unknown_short`: a long option, or a short cluster's first letter, that no argument of the walker's current argument list spells fails the parse with `UnknownFlag` of the word, whatever follows; `enter_step`: once a subcommand is selected, the current argument list is that subcommand's own. | An unknown letter after flags in a cluster (`-vx`). |
