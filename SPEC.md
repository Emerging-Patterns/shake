...
| ID | Requirement | Level | Status | Law |
| :---- | :---- | :---- | :---- | :---- |
| SHAKE-PARSE-1 | For every well-formed spec and word list, when `parse` succeeds, every binding in the Matched of each command on the selected path is `true` for a flag given while that command was current, a value-carrying piece of the words given while that command was current (a plain word bound to a positional, the value part of an option word, or the word after an option), bound verbatim under the name of the argument it was given for, or the default of an argument of that command that the words did not bind. The number of bindings that are neither flags nor defaults equals the number of value-carrying pieces. | Proved | pending | src/LAWS.bend values_given; src/LAWS.bend values_here |
...
SHAKE-PARSE-1 is the one pending row. Its partial laws: `values_given` proves that every binding in the Matched of a command on the selected path is `true`, a piece of one of the words, verbatim (what is left of a word once some chars are dropped from its front), or the default of one of that command's arguments; `values_here` proves each binding of a command is named after one of that command's own arguments, and that each piece comes from the words read while that command was current. Which argument each kind of word binds is proved word shape by word shape (SHAKE-TOK-1 to TOK-7, PARSE-2, PARSE-3). Left to prove: the count of pieces. The RFC's Rollout and [docs/rfc/shake-walker-proofs.md](docs/rfc/shake-walker-proofs.md) say in which phase each row's laws land. Every walker law holds wherever the words before the word it is about leave the walker in the state its premise names.
