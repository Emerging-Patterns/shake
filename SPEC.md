...
No row is pending. The RFC's Rollout and [docs/rfc/shake-walker-proofs.md](docs/rfc/shake-walker-proofs.md) say in which phase each row's laws land. Every walker law holds wherever the words before the word it is about leave the walker in the state its premise names.
