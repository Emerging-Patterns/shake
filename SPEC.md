...
SHAKE-PARSE-1 is the one pending row. Its partial law `values_given` proves that every binding in the Matched of a command on the selected path is `true`, a piece of one of the words, verbatim (what is left of a word once some chars are dropped from its front), or the default of one of that command's arguments: no value is invented, cut short or changed. Left to prove: that each piece comes from a word given while its command was current, under the name of the argument it was given for, and the count of pieces. The RFC's Rollout and [docs/rfc/shake-walker-proofs.md](docs/rfc/shake-walker-proofs.md) say in which phase each row's laws land. Every walker law holds wherever the words before the word it is about leave the walker in the state its premise names.
