...
| SHAKE-PARSE-2 | Plain words that select no subcommand bind the current command's positionals in spec order, one word each; a plain word with no positional left is refused as `Unexpected{word}`. | Proved | pending | src/LAWS.bend no_pos_left; src/LAWS.bend pos_next; src/LAWS.bend pos_binds |
...
| SHAKE-PARSE-7 | A successful parse binds every required argument of every command on the selected path, in that command's Matched. Otherwise the parse fails with `Missing{name}`. | Proved | pending |  |
...
| :---- | :---- | :---- |
| SHAKE-TOK-1 | `long_no_name`: a word `--=...` fails the parse with `UnknownFlag`, whatever follows; `long_binds`: a long option whose name and value the `=` cut gives binds the value, verbatim, to the option the name spells. | That `cut_eq` cuts at the first `=`: the laws take the cut as a premise. |
...
| SHAKE-PARSE-2 | `no_pos_left`: with no positional pending, a plain word that names no subcommand fails the parse with `Unexpected`, whatever follows; `pos_next`, `pos_binds`: otherwise it binds the next pending positional, and the ones after it stay pending in order. | A plain word `help` once a positional is bound (the laws take words other than `help`); and that the positionals pending when a command is entered are its own, in spec order, which is part of SHAKE-PARSE-4. |
| SHAKE-PARSE-5 | `choice_refused`, `value_refused`, `long_refused`, `raw_refused`: a value its argument's choices do not allow fails the parse with `BadValue`, whatever follows, for a positional before and after `--`, the word after an option, and a long option's `=` value; `pos_binds`, `raw_binds`, `value_binds`, `long_binds`: one they allow binds. | The same for a value glued to a short option (`-mx`, `-m=x`). |
...
| SHAKE-PARSE-10 | `repeated_long_flag`, `repeated_long_opt`: a long option that spells a flag or an `opt` option already given fails the parse with `Repeated` of its name, whatever follows; `long_binds`: an option built with `many` and spelled long binds each value after those given it before. | The same for short and clustered spellings. |
