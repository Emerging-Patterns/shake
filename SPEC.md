...
| SHAKE-TOK-5 | A flag given a value (`--verbose=x`, or `-v=x` in a cluster) is refused as `Unexpected{word}`. | Proved | pending | src/LAWS.bend flag_long_valued |
| SHAKE-TOK-6 | An option given no value in its own word takes the next word as its value, unless that word starts with `-` and is not `-`, or there is none; then the parse fails with `NoValue{name}`. | Proved | pending | src/LAWS.bend value_flag_shaped; src/LAWS.bend value_absent |
...
| SHAKE-PARSE-2 | Plain words that select no subcommand bind the current command's positionals in spec order, one word each; a plain word with no positional left is refused as `Unexpected{word}`. | Proved | pending | src/LAWS.bend no_pos_left |
...
| SHAKE-PARSE-5 | A value given to an argument with a nonempty choices list binds only when it is in the list, and otherwise fails with `BadValue{name, value}`. An argument with no choices accepts every value. | Proved | pending | src/LAWS.bend choice_refused |
...
| SHAKE-PARSE-10 | A flag or an option built with `opt` that is given a second time, under any of its spellings, fails the parse with `Repeated{at, name}`. An option built with `many` binds every value given, in the order given, and `get_all` reads them all. | Proved | pending | src/LAWS.bend repeated_long_flag; src/LAWS.bend repeated_long_opt |
...
Every pending row is unproved in full, except those below; the RFC's Rollout and [docs/rfc/shake-walker-proofs.md](docs/rfc/shake-walker-proofs.md) say in which phase each row's laws land. Every law below holds wherever the words before the word it is about leave the walker in the state its premise names.

| ID | Proved so far | Missing |
| :---- | :---- | :---- |
| SHAKE-TOK-1 | `long_no_name`: a word `--=...` fails the parse with `UnknownFlag`, whatever follows. | The binding half: that a long option binds the text after its first `=`, verbatim, to the argument its name spells. |
| SHAKE-TOK-5 | `flag_long_valued`: a long option that gives a flag not yet given a value fails the parse with `Unexpected`, whatever follows. | The same for `-v=x` in a short cluster. |
| SHAKE-TOK-6 | `value_flag_shaped`: a flag-shaped word where an option waits for its value fails the parse with `NoValue`, whatever follows; `value_absent`: so do words that end while it waits. | The accepted half: that any other next word binds as the option's value. |
| SHAKE-TOK-7 | `unknown_long`, `unknown_short`: a long option, or a short cluster's first letter, that no argument of the walker's current argument list spells fails the parse with `UnknownFlag` of the word, whatever follows. | An unknown letter after flags in a cluster (`-vx`); and that the walker's current argument list, after a subcommand is selected, is that subcommand's own, which is part of SHAKE-PARSE-4. |
| SHAKE-PARSE-2 | `no_pos_left`: with no positional pending, a plain word that names no subcommand fails the parse with `Unexpected`, whatever follows. | The binding half: that plain words bind the pending positionals in spec order, one each. |
| SHAKE-PARSE-5 | `choice_refused`: a plain word the next positional's choices do not allow fails the parse with `BadValue`, whatever follows. | The accepted half, and the same for an option's value. |
| SHAKE-PARSE-8 | `help_unknown`: a word after `help` that names no subcommand where the path stands fails the parse with `Unexpected`, whatever follows. | The path half: that `help` before any positional starts a help path, which subcommand names extend until the words end, with `NeedHelp` of the selected path and those names. |
| SHAKE-PARSE-10 | `repeated_long_flag`, `repeated_long_opt`: a long option that spells a flag or an `opt` option already given fails the parse with `Repeated` of its name, whatever follows. | The same for short and clustered spellings; and the `many` half, that an option built with `many` binds every value given, in order. |
